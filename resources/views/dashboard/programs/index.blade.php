...
@section('title', __('dashboard.module.programs') . ' — ' . __('brand.name'))
  ...
  <div class="flex flex-wrap items-center justify-between gap-3">
    <div>
      <h1 class="text-xl font-bold text-gray-900 dark:text-white sm:text-2xl">
        {{ __('dashboard.module.programs') }}
      </h1>
      <p class="mt-1 text-theme-sm text-gray-500 dark:text-gray-400">
        {{ __('dashboard.module.programs_desc') }}
      </p>
    </div>
    <a
      href="{{ route('dashboard.programs.create') }}"
        ...
        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
          @forelse ($programs as $program)
            <tr class="hover:bg-gray-50/50 dark:hover:bg-white/[0.02]">
              ...
              <td class="px-5 py-4">
                <code class="rounded bg-gray-100 px-1.5 py-0.5 text-xs text-gray-600 dark:bg-gray-800 dark:text-gray-400">{{ $program->key }}</code>
              </td>
              ...
              <td class="px-5 py-4">
                <div class="flex items-center justify-end gap-2">
                  ...
                  <form action="{{ route('dashboard.programs.destroy', $program) }}" method="POST" onsubmit="return confirm('{{ __('dashboard.confirm_delete') }}')">
                    @csrf
                    @method('DELETE')
                    <button
                      type="submit"
