...
@section('title', __('dashboard.module.facilities') . ' — ' . __('brand.name'))
  ...
  <div class="flex flex-wrap items-center justify-between gap-3">
    <div>
      <h1 class="text-xl font-bold text-gray-900 dark:text-white sm:text-2xl">
        {{ __('dashboard.module.facilities') }}
      </h1>
      <p class="mt-1 text-theme-sm text-gray-500 dark:text-gray-400">
        {{ __('dashboard.module.facilities_desc') }}
      </p>
    </div>
    <a
      href="{{ route('dashboard.facilities.create') }}"
          ...
          @forelse ($facilities as $facility)
            <tr class="hover:bg-gray-50/50 dark:hover:bg-white/[0.02]">
              ...
              <td class="px-5 py-4">
                ...
                @elseif ($facility->status === 'under_review')
                  <span class="inline-flex items-center rounded-full bg-warning-50 px-2.5 py-0.5 text-xs font-medium text-warning-700 dark:bg-warning-500/15 dark:text-warning-400">
                    {{ __('dashboard.status.under_review') }}
                  </span>
                @else
              ...
              <td class="px-5 py-4">
                <div class="flex items-center justify-end gap-2">
                  <a
                    href="{{ route('dashboard.facilities.edit', $facility) }}"
                  ...
                  <form action="{{ route('dashboard.facilities.destroy', $facility) }}" method="POST" onsubmit="return confirm('{{ __('dashboard.confirm_delete') }}')">
                    @csrf
                    @method('DELETE')
                    <button
                      type="submit"
