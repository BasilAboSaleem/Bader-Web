...
@section('title', __('dashboard.module.media') . ' — ' . __('brand.name'))
  ...
  <div class="flex flex-wrap items-center justify-between gap-3">
    <div>
      <h1 class="text-xl font-bold text-gray-900 dark:text-white sm:text-2xl">
        {{ __('dashboard.module.media') }}
      </h1>
      <p class="mt-1 text-theme-sm text-gray-500 dark:text-gray-400">
        {{ __('dashboard.module.media_desc') }}
      </p>
    </div>
    <a
      ...
      <table class="w-full">
        <thead>
          <tr class="border-b border-gray-100 dark:border-gray-800">
            ...
            <th class="px-5 py-4 text-start text-theme-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">
              {{ __('dashboard.field.file_path') }}
            </th>
            <th class="px-5 py-4 text-start text-theme-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">
              {{ __('dashboard.field.category') }}
            </th>
            <th class="px-5 py-4 text-start text-theme-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">
              {{ __('dashboard.field.consent') }}
            </th>
        ...
        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
          @forelse ($assets as $asset)
            <tr class="hover:bg-gray-50/50 dark:hover:bg-white/[0.02]">
              <td class="px-5 py-4">
                <p class="text-theme-sm font-medium text-gray-800 dark:text-white/90">{{ $asset->title_ar }}</p>
                @if ($asset->title_en)
                  <p class="text-xs text-gray-400 dark:text-gray-600" dir="ltr">{{ $asset->title_en }}</p>
                @endif
              </td>
              <td class="px-5 py-4 max-w-xs">
                <a href="{{ $asset->file_path }}" target="_blank" dir="ltr"
                  class="truncate block text-xs text-brand-500 hover:underline">
                  {{ Str::limit($asset->file_path, 50) }}
                </a>
              </td>
              ...
              <td class="px-5 py-4">
                ...
                @else
                  <span class="inline-flex items-center rounded-full bg-error-50 px-2.5 py-0.5 text-xs font-medium text-error-600 dark:bg-error-500/15 dark:text-error-400">
                    {{ __('dashboard.no') }}
                  </span>
                @endif
              </td>
              <td class="px-5 py-4">
                <div class="flex items-center justify-end gap-2">
                  <a href="{{ route('dashboard.media.edit', $asset) }}"
                  ...
                  <form action="{{ route('dashboard.media.destroy', $asset) }}" method="POST" onsubmit="return confirm('{{ __('dashboard.confirm_delete') }}')">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
          ...
          @empty
            <tr>
              <td colspan="5" class="px-5 py-12 text-center text-theme-sm text-gray-400 dark:text-gray-600">
