...
@section('title', __('dashboard.module.campaigns') . ' — ' . __('brand.name'))
  ...
  <div class="flex flex-wrap items-center justify-between gap-3">
    <div>
      <h1 class="text-xl font-bold text-gray-900 dark:text-white sm:text-2xl">
        {{ __('dashboard.module.campaigns') }}
      </h1>
      <p class="mt-1 text-theme-sm text-gray-500 dark:text-gray-400">
        {{ __('dashboard.module.campaigns_desc') }}
      </p>
    </div>
    <a
      ...
      <table class="w-full">
        <thead>
          <tr class="border-b border-gray-100 dark:border-gray-800">
            ...
            <th class="px-5 py-4 text-start text-theme-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">
              {{ __('dashboard.field.goal_amount') }}
            </th>
            <th class="px-5 py-4 text-start text-theme-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">
              {{ __('dashboard.field.raised_amount') }}
            </th>
            <th class="px-5 py-4 text-start text-theme-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">
        ...
        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
          @forelse ($campaigns as $campaign)
            <tr class="hover:bg-gray-50/50 dark:hover:bg-white/[0.02]">
              <td class="px-5 py-4">
                <p class="text-theme-sm font-medium text-gray-800 dark:text-white/90">{{ $campaign->title_ar }}</p>
                @if ($campaign->title_en)
                  <p class="text-xs text-gray-400 dark:text-gray-600" dir="ltr">{{ $campaign->title_en }}</p>
                @endif
              </td>
              <td class="px-5 py-4 text-theme-sm text-gray-500 dark:text-gray-400">
                @if ($campaign->goal_amount)
                  {{ number_format($campaign->goal_amount) }} {{ $campaign->currency_ar }}
                @else
                  —
                @endif
              </td>
              <td class="px-5 py-4 text-theme-sm text-gray-500 dark:text-gray-400">
                @if ($campaign->raised_amount)
                  {{ number_format($campaign->raised_amount) }} {{ $campaign->currency_ar }}
                  @if ($campaign->goal_amount > 0)
                    <span class="ms-1 text-xs text-brand-500">({{ round(($campaign->raised_amount / $campaign->goal_amount) * 100) }}%)</span>
                  @endif
                @else
                  —
                @endif
              ...
              <td class="px-5 py-4">
                <div class="flex items-center justify-end gap-2">
                  ...
                  <form action="{{ route('dashboard.campaigns.destroy', $campaign) }}" method="POST" onsubmit="return confirm('{{ __('dashboard.confirm_delete') }}')">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
